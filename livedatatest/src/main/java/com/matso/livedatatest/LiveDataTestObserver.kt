...
class LiveDataTestObserver<T> private constructor() : Observer<T?> {
     ...
     */
    fun valueHistory(): List<T> = Collections.unmodifiableList(valueHistory) as List<T>
    ...
    fun assertValue(valuePredicate: Function<T?, Boolean?>): LiveDataTestObserver<T> {
        ...
        if (!valuePredicate.apply(value)!!) {
            throw fail(
                "Value " + valueAndClass(
                    value
                ) + " does not match the predicate " + valuePredicate.toString() + "."
            )
        }
        return this
    }
    ...
    fun assertValueHistory(vararg values: T): LiveDataTestObserver<T> {
        ...
        if (size != values.size) {
            throw fail(
                "Value count differs; expected: " + values.size + " " + values.contentToString() + " but was: " + size + " " + this.valueHistory
            )
        }
    ...
    fun assertNever(valuePredicate: Function<T, Boolean>): LiveDataTestObserver<T> {
        ...
        for (valueIndex in 0 until size) {
            ...
            if (valuePredicate.apply(value)) {
                throw fail("Value at position $valueIndex matches predicate $valuePredicate, which was not expected.")
            }
        }
        return this
    }
    ...
    fun <N> map(mapper: Function<T?, N>): LiveDataTestObserver<N> {
        val newObserverLiveData: LiveDataTestObserver<N> =
            create()
        // We want the history match the current one
        for (value in valueHistory) {
            newObserverLiveData.onChanged(mapper.apply(value))
        }
        doOnChanged(Map(newObserverLiveData, mapper))
        return newObserverLiveData
    }
    ...
    internal class Map<T, N>(
        private val newObserverLiveData: LiveDataTestObserver<N>,
    ...
    companion object {
        ...
        fun <T> test(liveData: LiveData<T>): LiveDataTestObserver<T> {
            val observerLiveData: LiveDataTestObserver<T> =
                LiveDataTestObserver()
            liveData.observeForever(observerLiveData)
            return observerLiveData
        }
    }
}
